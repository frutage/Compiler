...
avltree  avltree_insertNode(avltree tree,elementType key)
{
    ...
    else if(strcmp(key.name, tree->key.name) < 0)//在左子树中插入结点
    {
        ...
        {
            //在这儿判断是LL还是LR
            if(strcmp(key.name, tree->left->key.name) < 0)
            {
                //LL旋转
                tree= left_left_rotation(tree);
	...
	void print_avltree(avltree tree, elementType key, int direction)
	{
		...
		if(direction==0)    // tree是根节点
            ...
            printf("%s is %s's %6s child\n", tree->key.name, key.name, direction==1?"right" : "left");
...
{
    if(tree==NULL||strcmp(tree->key.name, key.name) == 0)
    {
        return tree;
    }
    else if(strcmp(key.name, tree->key.name) < 0)
    {
...
avltree  avltree_deleNode(avltree tree, elementType key)
{
    ...
    if(strcmp(key.name, tree->key.name) < 0)//要删除的结点在左子树
    {   //递归找到要删除的结点
    ...
    else if(strcmp(key.name, tree->key.name) > 0) //要删除的结点在右子树
    {
...
avlnode *search_sym(stNode* cur, elementType key) {
  while(cur != NULL) {
    avlnode* n = search_node(cur->tree, key);
    if (n != NULL) {
      return n;
    }
    cur = cur->pre;
